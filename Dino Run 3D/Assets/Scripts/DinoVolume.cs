...
public class DinoVolume : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.RightArrow) && dinoManager.isPaused && dinoManager.canAdjustVolume || Input.GetMouseButtonDown(1) && dinoManager.isPaused && dinoManager.canAdjustVolume)
        {
            SetVolume(currentVolume + 1); // Increase volume
            PlayVolumeChangeSound();
        }
        else if (Input.GetKeyDown(KeyCode.LeftArrow) && dinoManager.isPaused && dinoManager.canAdjustVolume || Input.GetMouseButtonDown(0) && dinoManager.isPaused && dinoManager.canAdjustVolume)
        {
    ...
    {
        AudioSource audioSource = audioSources[audioSources.Length - 4];
        audioSource.Play();
    }
