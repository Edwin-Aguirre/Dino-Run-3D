...
public class DinoVolume : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.RightArrow) || Input.GetMouseButtonDown(1) && dinoManager.isPaused && dinoManager.canAdjustVolume)
        {
            SetVolume(currentVolume + 1); // Increase volume
            PlayVolumeChangeSound();
        }
        else if (Input.GetKeyDown(KeyCode.LeftArrow) || Input.GetMouseButtonDown(0) && dinoManager.isPaused && dinoManager.canAdjustVolume)
        {
